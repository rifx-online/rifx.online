...
export const freeItems = (allPosts: any[]) => {
  const freePosts = allPosts
    .filter((post) => post.data.is_free && post.data.is_active)
    .sort((a, b) => new Date(b.data.last_updated).getTime() - new Date(a.data.last_updated).getTime());
  return freePosts;
};
...
export const discountItems = (allPosts: any[]) => {
  const discountPosts = allPosts
    .filter((post) => post.data.discount < 1 && post.data.is_active)
    .sort((a, b) => new Date(b.data.last_updated).getTime() - new Date(a.data.last_updated).getTime());
